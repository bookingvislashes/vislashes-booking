...
It opens with `public/email/confetti.gif`, regenerated by
`scripts/generate-confetti.py`. An animated GIF rather than CSS, because no
mail client runs CSS animation: Gmail, Apple Mail, iOS Mail and Yahoo play it,
Outlook on Windows shows frame 0 and stops — so frame 0 is drawn to look
finished on its own. Drawn in the brand palette rather than party colours.

This is the only email here whose subject carries an emoji. Everywhere else
they are left off because they cost a little deliverability for nothing; on a
once-a-year card it is worth it, and the 🎉 in the subject and heading is the
part that still lands when a client's inbox blocks the confetti.
